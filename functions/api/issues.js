/**
 * Issue Finder API - Cloudflare Pages Function
 *
...
const CACHE_TTL_HOT = 600;    // 10 min
const CACHE_TTL_COLD = 1800;  // 30 min
...
export async function onRequest(context) {
  ...
  try {
    ...
    context.waitUntil(cache.put(cacheKey, responseToCache.clone(), { expirationTtl: ttl }));
...
function getCacheTTL(params) {
  if (HOT_QUERIES.includes(params.language)) {
    return CACHE_TTL_HOT;
  }
  return CACHE_TTL_COLD;
}
